...
class Block {
 public:
  ...
  const char* limit() const { return data_ + size_; }
 ...
 protected:
  ...
  uint32_t size_;         // contents_.data.size()
...
class ParsedDataBlock : public DataBlock {
 public:
  // Initialize the block with the specified contents.
  explicit ParsedDataBlock(BlockContents&& contents,
                           size_t read_amp_bytes_per_bit = 0,
                           Statistics* statistics = nullptr);
...
class DataBlockIter final : public BlockIter<Slice> {
 public:
  ...
  void Initialize(const Comparator* raw_ucmp, const DataBlock* data_block,
    ...
    last_bitmap_offset_ = data_block->size();
  }
