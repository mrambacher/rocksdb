...
Slice Block::DecodeKeyAtRestart(uint32_t index) const {
  ...
  const char* ptr =
      DecodeKV(data_ + offset, limit(), &shared, &non_shared, &value_length);
...
uint32_t Block::ParseKVBefore(uint32_t offset, IterKey* key, bool* is_shared,
  ...
  if (offset == 0) {  // First key
    return size_;     // Return last to signify end
  }
...
void IndexBlockIter::DecodeCurrentValue(bool is_shared) {
  Slice v(value_.data(), block_->limit() - value_.data());
...
Block::Block(BlockContents&& contents)
    : contents_(std::move(contents)),
      ...
      size_(static_cast<uint32_t>(contents_.data.size())),
      restarts_(nullptr),
      num_restarts_(0) {
...
IndexBlock::IndexBlock(BlockContents&& contents, bool value_delta_encoded)
  ...
  if (size_ > 0) {
    ...
    if (restarts_ < data_) {
      ...
      size_ = static_cast<uint32_t>(restarts_ - data_);
    }
  }
}
...
DataBlock::DataBlock(BlockContents&& contents, size_t read_amp_bytes_per_bit,
    ...
    switch (index_type_) {
      ...
      case BlockBasedTableOptions::kDataBlockBinaryAndHash: {
        size_t map_size = DataBlockHashIndex::Create(
            data_, size_ - sizeof(uint32_t), &data_block_hash_index_);
        if (map_size == 0 || map_size > size_) {
  ...
  if (read_amp_bytes_per_bit != 0 && statistics && size_ != 0) {
    read_amp_bitmap_.reset(
        new BlockReadAmpBitmap(size_, read_amp_bytes_per_bit, statistics));
  }
}

ParsedDataBlock::ParsedDataBlock(BlockContents&& contents,
...
uint32_t ParsedDataBlock::GetRestartPoint(uint32_t index) const {
...
Slice ParsedDataBlock::DecodeKeyAtRestart(uint32_t index) const {
  if (index < restart_indices_.size()) {
...
uint32_t ParsedDataBlock::ParseKVAfter(uint32_t offset, IterKey* key,
...
uint32_t ParsedDataBlock::ParseKVBefore(uint32_t offset, IterKey* key,
                                        bool* shared, Slice* value) const {
  if (offset > 0) {
...
size_t ParsedDataBlock::ApproximateMemoryUsage() const {
  return DataBlock::ApproximateMemoryUsage() + key_buff_.size();
}

uint32_t ParsedDataBlock::GetOffsetForEntry(uint32_t entry) const {
  if (entry == 0) {
    return 0;
...
void ParsedDataBlock::MarkReadAmpBitMap(uint32_t current, uint32_t next) const {
...
uint32_t ParsedDataBlock::TEST_CurrentEntrySize(uint32_t current,
                                                uint32_t next) const {
  uint32_t start = GetOffsetForEntry(current);
  uint32_t end = GetOffsetForEntry(next);
  return end - start;
